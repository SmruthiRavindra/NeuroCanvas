...
import { Music, Palette, FileText, Sparkles, RefreshCw, Mic, Square, Send, Play, Pause, StopCircle } from 'lucide-react';
...
export default function CreativeCanvas() {
  ...
  const lyriaStream = useLyriaStream();
  ...
  const generateAudioMutation = useMutation({
    mutationFn: async (data: { prompt: string; personaId: string; compositionType: string; mood?: string }) => {
    ...
    onSuccess: (data: any) => {
      if (data.usesLyria && data.musicConfig) {
        // Display music configuration
        const configText = `
🎼 Music Generated by ${data.persona}!

Mood: ${data.musicConfig.mood}
BPM: ${data.musicConfig.bpm}
Genres: ${data.musicConfig.genres.join(', ')}

Musical Elements:
${data.musicConfig.prompts.map((p: any) => `  • ${p.text} (${Math.round(p.weight * 100)}%)`).join('\n')}

Note: This configuration is ready for Lyria RealTime music generation. Full implementation requires WebSocket integration for real-time audio streaming.
        `.trim();

        toast({
          title: "🎵 Music configuration generated!",
        ...
        // For demonstration, speak the configuration
        if ('speechSynthesis' in window) {
          const announcement = `Music generated with ${data.persona} persona. ${data.musicConfig.bpm} BPM, ${data.musicConfig.mood} mood. Genres: ${data.musicConfig.genres.slice(0, 2).join(' and ')}`;
        ...
        toast({
          ...
          description: `${data.persona} configuration prepared`,
        });
      }
    },
    onError: (error: any) => {
  ...
  const handleGenerateVoice = async () => {
    ...
    try {
      // Connect to Lyria if not connected
      if (!lyriaStream.isConnected) {
        lyriaStream.connect();
      ...
      if (!response.ok) throw new Error('Failed to get music config');
      
      const data = await response.json();

      if (data.musicConfig) {
        // Configure Lyria with persona settings
        lyriaStream.configure({
          prompts: data.musicConfig.prompts,
          bpm: data.musicConfig.bpm,
          temperature: data.musicConfig.temperature
        });
        ...
        toast({
          ...
          description: `${data.persona} | ${data.musicConfig.bpm} BPM | ${data.musicConfig.mood}`,
          duration: 5000,
        });
      }
    } catch (error: any) {
                    ...
                    <div className="mt-4 space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          disabled={lyriaStream.isLoading || lyriaStream.isPlaying || !userInput.trim()}
                          className="gap-2 flex-1"
                          data-testid="button-generate-voice"
                        >
                          <Play className="w-4 h-4" />
                          {lyriaStream.isLoading ? 'Connecting...' : lyriaStream.isPlaying ? 'Playing...' : 'Generate Music'}
                        </Button>
                        
                        {lyriaStream.isPlaying && (
                          <>
                            <Button
                              variant="outline"
                              size="icon"
                              onClick={() => lyriaStream.pause()}
